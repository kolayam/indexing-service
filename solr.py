...
for s in g.subjects(RDF.type, OWL.Class):
    ...
    print(f"Class URI: {s}, Label: {label}")
...
for s in g.subjects(RDF.type, OWL.DatatypeProperty):
    ...
    #     "nameSpace" : namespace,
    #     "range" : g.value(s, RDFS.range),
    #     "visible" : boolean_value_of(visible),
    #     "required" : boolean_value_of(required),
    #     "label" : g.value(s, RDFS.label),
    #     "comment" : g.value(s,RDFS.comment),
    #     "propType":propType,
    #     "valueQualifier": valueQualifier,
...
for s in g.subjects(RDF.type, OWL.FunctionalProperty):
    label = g.value(s, RDFS.label)
    # print(f"class {s} Subclasses of {check_property(g,s,NIMBLE.QuantityProperty,NIMBLE.QuantityProperty)}")
    # print(f"Functional Property URI: {s}, Label: {label}")

# Query for properties and their domains
# print("Properties and Their Domains:")
# for prop in g.subjects(RDF.type, OWL.ObjectProperty):  # For Object Properties
#     domain = g.value(prop, RDFS.domain)
#     print(f"Property: {prop} -> Domain: {domain}")

for prop in g.subjects(RDF.type, OWL.DatatypeProperty):  # For Datatype Properties
    domain = g.value(prop, RDFS.domain)
    # print(f"Property: {prop} -> Domain: {domain}")

# Example: Exporting as JSON for Solr or Other Uses
# data = []
# for s in g.subjects(RDF.type, OWL.Class):
#     label = g.value(s, RDFS.label)
...
# Save to a JSON file
# import json
# with open("eclass_data.json", "w") as f:
